...
const addAction = () => {
    _navigate(
        `${basePath}/ad_categories/add${_parentId === 0 ? "" : "/" + _parentId}`
    );
};

const childrenAction = (item) => {
    if (!isNaN(item?.id) && item?.id > 0) {
        _navigate(`${basePath}/ad_categories/${item.id}`);
    } else {
        _navigate(`${basePath}/ad_categories`);
    }
};

const editAction = (item) => {
    if (!isNaN(item?.id) && item?.id > 0) {
        _navigate(`${basePath}/ad_categories/edit/${item.id}`);
    }
};
...
const fetchAdCategories = async (data = null) => {
